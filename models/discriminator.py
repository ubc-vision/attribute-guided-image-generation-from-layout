...
class AttributeDiscriminator128(nn.Module):
    def __init__(self, conv_dim=64, downsample_first=False, n_attribute = 128):
        super(AttributeDiscriminator128, self).__init__()
...
class ImageDiscriminator(nn.Module):
    def __init__(self, conv_dim=64):
        ...
        # 2x upsampling
        # (3, 128, 128) -> (64, 64, 64)
        # self.up1 = OptimizedBlock(3, self.ch, downsample=True),
        # # (64, 64, 64) -> (64, 32, 32)
        # self.up2 = ResidualBlock(self.ch, self.ch, downsample=True),
        self.main = nn.Sequential(
            # (3, 64, 64) -> (64, 32, 32)
        ...
        # self.main = nn.Sequential()
        # self.main.add_module("0", OptimizedBlock(3, self.ch, downsample=True))
        # # self.main.add_module("2x_opt0", OptimizedBlock(3, self.ch, downsample=True))
        # # self.main.add_module("2x_res0", ResidualBlock(self.ch, self.ch, downsample=True))
        # self.main.add_module("1", ResidualBlock(self.ch, self.ch * 2, downsample=True))
        # self.main.add_module("2", ResidualBlock(self.ch * 2, self.ch * 4, downsample=True))
        # self.main.add_module("3", ResidualBlock(self.ch * 4, self.ch * 8, downsample=True))
        # self.main.add_module("4", ResidualBlock(self.ch * 8, self.ch * 16, downsample=True))
        # self.main.add_module("5", ResidualBlock(self.ch * 16, self.ch * 16, downsample=True))
...
class ObjectDiscriminator(nn.Module):
    def __init__(self, conv_dim=64, n_class=0, downsample_first=False, n_attribute = 128):
        super(ObjectDiscriminator, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        self.main = nn.Sequential(
            ################ IMPORTANT: ############Real dim: (3, 32, 32) -> (64, 32, 32)
            # (3, 64, 64) -> (64, 64, 64)
            OptimizedBlock(3, conv_dim, downsample=downsample_first),
            # (64, 64, 64) -> (128, 32, 32)
            ResidualBlock(conv_dim, conv_dim * 2, downsample=True),
